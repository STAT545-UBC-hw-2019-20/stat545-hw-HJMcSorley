...
Using dplyr and the piping operator, this code summarizes the gapminder data set into each continent's minimum and maximum GDP per capita, and notes the country associated with each value. 
```{r}
str(gapminder)  # structure

# minimum and maximum GDP per capita for all continents
# add the corresponding country for the min/max also
# add a GDP variable (GDP/captia * population)
# sort from smallest to largest
gapminder %>% 
  group_by(continent) %>% 
  summarize(gdpPercap_min = min(gdpPercap),
            country_min = country[gdpPercap == gdpPercap_min],
            gdpPercap_max = max(gdpPercap), 
            country_max = country[gdpPercap == gdpPercap_max]) %>% 
  arrange(gdpPercap_min)


# plot continents with their GDP per capita 
# hightlight the min and max with box plots
# y axis on a log scale
gapminder %>%
  ggplot(aes(x = continent, y = gdpPercap, group = continent, size = gdpPercap == min(gdpPercap))) +
  #geom_boxplot() +
  geom_jitter(aes(fill = continent), alpha = 0.5, shape = 21) +
  scale_y_log10("GDP per capita", labels = scales::dollar_format()) + 
  theme_light() 

# I was trying to add additional markers on the plot for continental min and max gdp... but can only get one (min)
```
